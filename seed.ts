...
async function main() {
  ...
  const reportDescriptions = [
    "Construction work blocking left lane",
  ...
  const spotsData = Array.from({ length: 20 }).map(() => {
    const loc = pick(dhakaLocations);
    return {
  ...
  const savedData = Array.from({ length: 20 }).map(() => ({
